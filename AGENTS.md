...
Before opening a PR (or handing off work), verify:

1. **Behavior**: The feature works on the target platform(s) and does not regress existing flows
2. **Quality gates**: Run `yarn lint`, `yarn typecheck`, and relevant `yarn test` suites
3. **i18n**: All new user-facing strings are wrapped (`msg`, `plural`, or `<Trans>`)
4. **Accessibility**: Interactive controls have clear `label` values and sensible hints where needed
5. **Loading/Error states**: Async UI handles loading, empty, and error states explicitly
6. **Tests**: Add or update tests for behavior changes (especially bug fixes)
7. **Scope discipline**: Keep changes focused; avoid drive-by refactors unless necessary

## Error Handling and Observability

Use this pattern for async/mutation code:

1. Handle expected/network failures gracefully in UI without noisy logging
2. Handle known typed XRPC errors with specific user-friendly behavior when possible
3. Log unexpected failures with contextual metadata (prefer `safeMessage`)
4. Never swallow errors silently; either surface to users or log for diagnosis
...
onError: error => {
  if (isNetworkError(error)) {
    // Show retry affordance or inline message
  } else if (error instanceof SomeTypedError) {
    // Handle known API error
  } else {
    logger.error('Unexpected failure in FeatureX', {safeMessage: error})
  }
}
```
...
When reviewing a change, prioritize these checks first:

1. **Dialog sequencing**: Actions after dialog dismissal use `control.close(() => ...)`
2. **Translations**: No raw user-facing strings slipped in
3. **Platform behavior**: Web/native differences are handled intentionally
4. **Query hygiene**: Correct query keys, invalidation, stale times, and pagination cursor usage
5. **Import hygiene**: Uses `#/` aliases (no deep relative imports)
6. **React Compiler alignment**: No unnecessary `useMemo`/`useCallback`
7. **Test coverage**: Behavior change has at least one validating test path
